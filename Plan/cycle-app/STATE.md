...
**Last reviewed commit:** 55db0c4 (reviewed by ca6a). Unreviewed tally: 0 lines. Next slice is **ca9** (cold-start offline — the app cannot open without a signal), then **ca8**, the final review over `1065c1e...HEAD`.
## Done
...
- **A Note cell that already holds a formula is normalised, not recovered** (ca6a) — `writeRow` reads the row with `getValues()`, so `=1+1` comes back as `2` and the RAW write stores the text `"2"`. ca5a stops new ones; not worth a `getFormulas()` read on every write.
- **`unreadableDates` under-reports** (ca6a) — the date scan `break`s on the matched row, so odd date cells below it are never collected. The warning only has to send you to look at the sheet.
